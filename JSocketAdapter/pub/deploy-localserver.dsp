...
<title>Deploy Local Server Socket</title>
		...
		<script language="javascript">
			function doAction(todo) {				
				...
				if (todo == 'search') {
					...
					if (document.pform.selectedList.value == "") {
						alert("Deploy할 Socket Port를 선택하십시요!!!");
						return;
					} else {
						document.pform.submit();
					}
...
<body onload="javascript:alertMsg()">
		...
		%else%
		<div class="message">Target Server에서 Port Number에 대한 정보가 다를 경우 Deploy 한 후에 Target Server에서 직접 수정을 해야 합니다.</div>
		%endifvar%
		...
		%ifvar deploy equals('remote')%
			...
			<ul class="listitems">
				<li class="listitem"><a href="deploy-localserver.dsp?deployClass=%value deployClass%&searchType=noDeploy&dupProcessType=notAdd">Return to Local Server Socket Deployment Information</a></li>
			</ul>
			
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Local Server Socket Deployment Result</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
        	...
        	<tr>
						<td class="heading" colspan=5>Local Server Socket Information</td>
					</tr>
       ...
       %endifvar%
		...
		%else%
			...
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Local Server Socket Deployment</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="action" colspan="2">
						<input type="button" name="SUBMIT" value="Search Local Server Socket"  onclick="return doAction('search')"></input>&nbsp;&nbsp;
						<input type="button" name="SUBMIT" value="Deploy Local Server Socket"  onclick="return doAction('remote')"></input>
					</td>
				</tr>
			</table>
      ...
      <table class="tableView" width=100%>
        <tr>
					<td class="heading" colspan=9>Local Server Socket Information</td>
				</tr>
        <tr class="subheading2">
				...
				%ifvar -notempty RegInfoList%
					...
					%endloop%
				%else%
					<tr><td colspan=9>No local server sockets to deploying are currently.</td></tr>
				%endif%
    	</table>
		%endifvar%
	</form>
