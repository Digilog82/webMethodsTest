	...
	<head>		
		...
		<script language="javascript">
			...
			function checkServiceNode(aTodo) {
				//var variableUrlService = $("#variableUrlService").val();
				$("#todo").val(aTodo);
				...
				$.ajax({
					...
					cache : false,
					//data : "todo=update&variableUrlService=" + variableUrlService,
					data : $("form").serialize(),
					dataType : "json",
					async : true,
					...
					error : function(jqXHR, textStatus, errorThrown) {
								alert("Service Node 존재여부를 체크하는 중에 에러가 발생하였습니다 !!!\nHttp Status : " + jqXHR.status + "\nStatus Msg : " + jqXHR.statusText);
					        }
				});
			};
			
			function downloadSchemaDefine(fileName) {
				var moveAction = $("#moveAction").val();
				var filePath = $("#filePath").val();
				
				$.ajax({
					type : "POST",
					url : "/invoke/JSocketAdapter.ADMIN/downloadFile",
					cache : false,
					data : "moveAction=" + moveAction + "&filePath=" + filePath + "&fileName=" + fileName,
					dataType : "bytes",
					async : true,
					success : function(result) {
								if (result.alertMsg != "") {
					...
					error : function(jqXHR, textStatus, errorThrown) {
								alert("Schema Define 엑셀 파일을 다운로드하는 중에 에러가 발생하였습니다 !!!\nHttp Status : " + jqXHR.status + "\nStatus Msg : " + jqXHR.statusText);
					        }
				});
			};
			...
			function doAction(todo, dname){
				...
				if (todo == 'delete') {
					...
					*/
					...
					var schemaDefineFileName = frm.schemaDefineFileName.value;
					...
					var sourceSystemNames = frm.tempSourceSystemName;
					var sLength = sourceSystemNames.options.length;
					var sourceSystemNameList = "";
					var docInterfaceType = frm.docInterfaceType.value;
					var requestResponseType = frm.requestResponseType.value;
					...
					for (var i = 0; i < sLength; i++) {
						if (sourceSystemNames.options[i].selected == true) {
							if (sourceSystemNameList == "") {
								sourceSystemNameList = sourceSystemNames.options[i].value;
							} else {
								sourceSystemNameList = sourceSystemNameList + "/" + sourceSystemNames.options[i].value;
							}
						}
					}
					
					frm.sourceSystemName.value = sourceSystemNameList;
					...
					// 전문정보 등록 시 API Portal에 API 자동 등록 기능이 구현되어 있을 경우에는 Target HTTP/S URL을 반드시 입력해야 한다.
					// API 자동 등록 기능이 구현되어 있지 않을 경우에는
					// Internal API Gateway 서버가 존재하지 않을 경우에는 Target HTTP/S URL을 반드시 입력해야 한다.
					// Internal API Gateway 서버가 존재하면서 여러 개의 전문이 하나의 공통 Internal API URL을 사용하는 경우에는 Target HTTP/S URL을 반드시 입력해야 한다.
					// 그러나 조건 따지지 않고 Socket To REST API 방식인 경우에는 정보 관리 차원에서 Target HTTP/S URL을 반드시 입력하도록 한다.
					if (docInterfaceType == "Socket To REST API" && requestResponseType != "response") {
						if (targetUrl == "") {
					...
					if (schemaDefineFileName != "") {
						...
						*/
						...
						// 파일 업로드
						let uploadSuccess = uploadSchemaDefFile(classCode + businessCode, interfaceID);
						
						// 파일 업로드 성공한 경우에 Submit
						if (uploadSuccess == "true") {						
							if (docInterfaceType != "REST API To Socket") {
								frm.socketProcessServiceUrl.value = "";
							} else if (docInterfaceType == "REST API To Socket") {
					...
					*/
					...
					if (schemaDefineFileName != "") {
						...
						*/
						...
						// 파일 업로드
						let uploadSuccess = uploadSchemaDefFile("%value docName%", "%value interfaceID%");
					...
					/* 2022.08.15 수정. iframe을 사용하지 않고 JQuery를 사용하는 방식으로 변경(함수 calDocLength() 호출)
					var customerCode = "%value customerCode%";
			...
			function uploadSchemaDefFile(docName, interfaceID) {
				let uploadSuccess = "true";
				var formData = new FormData();
				formData.append("docName", docName);
				formData.append("interfaceID", interfaceID);
				formData.append("socketServerType", "local");
				formData.append("schemaDefineFileName", $("#sdFile").prop("files")[0]);
				
				$.ajax({
					type : "POST",
					enctype : "multipart/form-data",
					url : "/invoke/JSocketAdapter.ADMIN/uploadSchemaDefFile",
					cache : false,
					data : formData,
					processData: false,
					contentType: false,
					...
					error : function() {
								alert("Schema Define File을 업로드하는 중에 에러가 발생하였습니다 !!!");
								uploadSuccess = "false";
					        }
				});
			...
			function checkUpload() {
				var upload = document.pform.schemaDefUpload.value;
			...
			function calDocLength() {
				var customerCode = "%value customerCode%";
				var recCountFieldName = "";
				
				// 현대제철 전용
				if (customerCode == "HSC") {					
					recCountFieldName = document.pform.recCountFieldName.value;
				}
				
				var schemaDefine = document.pform.schemaDefine.value;
				...
				if (arrayCountYN == "" || arrayCountYN == "Y") {
					// fieldDefine[3] 위치에 Array Count YN 항목이 있는 경우
					existArrayCount = "true";
				} else {
					// fieldDefine[3] 위치에 Data Type 항목이 있는 경우
					existArrayCount = "false";
				}
				
				calDocLength2(recCountFieldName, existArrayCount, schemaDefine);
			}
			
			function calDocLength2(recCountFieldName, existArrayCount, schemaDefine) {
				$.ajax({
					type : "POST",
					url : "/invoke/JSocketAdapter.COMMON.UTIL.FLOW/getSimpleDocLengthSum2",
					cache : false,
					data : "recCountFieldName=" + recCountFieldName + "&existArrayCount=" + existArrayCount + "&schemaDefine=" + schemaDefine,
					dataType : "json",
					async : true,
					success : function(result) {
								if (result.alertMsg != "") {
								...
								$("#docLength").val(result.lengthSum);
							  },
					error : function() {
								alert("전문길이 계산하는 중에 에러가 발생하였습니다 !!!");
					        }
				});
			...
			function docDataView(docFormType) {
				// javascript를 이용해서 팝업을 post 방식으로 띄우는 샘플
				
				var popupTitle = "docDataView";
				var docForm;
				
				if (docFormType == "request") {
					// 요청전문을 파싱하는 경우
					docForm = document.requestDocForm;
				} else {
					// 응답전문을 파싱하는 경우
					docForm = document.responseDocForm;
				}
				
				window.open("", popupTitle, "directories=no,location=yes,menubar=yes,scrollbars=yes,status=yes,toolbar=yes,resizable=yes", true);
				
				docForm.target = popupTitle;
				docForm.action = "basic-docdatadetail.dsp";
				docForm.submit();
			}
			
			function docDataView2(docFormType) {
				// jquery를 이용해서 팝업을 post 방식으로 띄우는 샘플
				...
				if (docFormType == "request") {
					$("#requestDocForm").attr("target", popupTitle);
					$("#requestDocForm").attr("action", "basic-docdatadetail.dsp");
					$("#requestDocForm").submit();
				} else {
					$("#responseDocForm").attr("target", popupTitle);
					$("#responseDocForm").attr("action", "basic-docdatadetail.dsp");
					$("#responseDocForm").submit();
				}
			};
			...
			function socketProcessUrlDisplay(aType) {
				var requestResponseType = $("#requestResponseType").val();
				
				if (aType == "REST API To Socket" && requestResponseType != "response") {
					$("#socketProcessServiceUrlTr").show();
				} else {
					$("#socketProcessServiceUrlTr").hide();
				}
			};
			
			function socketProcessUrlDisplay2(aType) {
				var docInterfaceType = $("#docInterfaceType").val();
				
				if (docInterfaceType == "REST API To Socket" && aType != "response") {
					$("#socketProcessServiceUrlTr").show();
				} else {
					$("#socketProcessServiceUrlTr").hide();
				}
	...
	<body onload="javascript:alertMsg(); initBusinessName()">
		...
		<table class="tableForm" width="100%">	
			%ifvar todo equals('create')%
				...
				<tr>
					<td class="evenrow">Plant</td>
					<td class="evenrow-l">							
						<select name="plantCode" style="width:400">
				...
				%ifvar customerCode equals('HCC')%
				<tr>
					<td class="evenrow" width="30%">Target Doc Name</td>
					<td class="evenrow-l" width="70%"><input type="text" name="targetDocName" size="20" style="font-size:10pt;width:400"></td>
				</tr>
				%endifvar%
				<!-- 현대제철 냉연공장 전용 -->
				
				<tr>
					<td class="evenrow" width="30%">Doc Request/Response Type</td>
					<td class="evenrow-l" width="70%">
						<select id="requestResponseType" name="requestResponseType" style="width:200" onchange="socketProcessUrlDisplay2(this.value)">
							<option value="requestResponse">Request & Response</option>
							<option value="onlyRequest">Only Request</option>
							<option value="response">Response</option>
						</select>&nbsp;Response Doc Name&nbsp;<input type="text" name="resDocName" size="20" style="font-size:10pt;width:90">
					</td>
				</tr>
				<tr>
					<td class="evenrow" width="30%">Doc Interface Type</td>
					<td class="evenrow-l" width="70%">
						<select id="docInterfaceType" name="docInterfaceType" style="width:200" onchange="socketProcessUrlDisplay(this.value)">
				...
				<tr style="display:none" id="socketProcessServiceUrlTr">
					<td class="evenrow" width="30%">Socket Processing Service URL</td>
					<td class="evenrow-l" width="70%"><input type="text" name="socketProcessServiceUrl" value="%value socketProcessUrl%" size="20" style="font-size:10pt;width:400"></td>
				</tr>				
				<tr>
				...
				<tr>
					<td class="evenrow" width="30%">Answerback Doc YN</td>
					<td class="evenrow-l" width="70%">
						...
						%ifvar customerCode equals('HSC')%
							...
							Answerback Doc ID&nbsp;<input type="text" name="ackDocID" value="" size="20" style="font-size:10pt;width:100">&nbsp;&nbsp;&nbsp;&nbsp;
							Answerback Socket Name&nbsp;
				...
				<tr>
					<td class="evenrow" width="30%">Processing Service</td>
					<td class="evenrow-l" width="70%">
						<select name="targetService" style="width:400">
							%loop mainServiceList%
								<option value="%value mainServiceList%">%value mainServiceList%</option>
							%endloop%
						</select>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="evenrow-l" width="70%">
						...
						Answerback Doc Data&nbsp;<input type="text" name="ackDocData" value="" size="20" style="font-size:10pt;width:100">&nbsp;&nbsp;&nbsp;&nbsp;
						Answerback Receive Retry Count&nbsp;<input type="text" name="ackRetryCount" value="" size="20" style="font-size:10pt;width:100">						
					</td>
				</tr>					
				<tr>
					<td class="evenrow" width="30%">Messaging Use YN</td>
				...
				<tr>
					<td class="evenrow" width="30%">Source System Name</td>
					<td class="evenrow-l" width="70%">
						<select name="tempSourceSystemName" style="width:400" size=5 multiple>
							%loop SystemNameDescList%
								<option value="%value systemNameValue%">%value systemNameDisplay%</option>
							%endloop%
						</select>
						<input type="hidden" name="sourceSystemName" value="">
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="evenrow" width="30%">External API URL</td>
					<td class="evenrow-l" width="70%"><input type="text" name="externalApiUrl" value="" size="20" style="font-size:10pt;width:400"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="evenrow-l" width="70%">
						<input type="file" id="sdFile" name="schemaDefineFileName" style="font-size:10pt;width:500" onclick="checkUpload()">
					</td>
				</tr>
				...
				<tr>
					...
					<td class="evenrow-l" width="70%"><input type="text" id="docLength" name="docLength" size="20" style="font-size:10pt;width:400">&nbsp;<input type="button" name="SUBMIT" value="Calculate Doc Length"  onclick="calDocLength()"></input></td>
				</tr>
				<tr>
				...
				<tr>
					<td class="action" colspan="2">
						...
						<input type="button" name="SUBMIT" value="Create Interface Doc" onclick="checkServiceNode('add')"></input>
					</td>
				</tr>
			%else%
				%ifvar todo equals('read')%
					...
					<tr>
						...
						<td class="evenrow-l">							
							...
							<script language="javascript">
								document.pform.plantCode.value = "%value plantCode%";
							</script>
						</td>
					</tr>
					%endifvar%
					...
					<tr>
						<td class="evenrow" width="30%">Target Doc Name</td>
						<td class="evenrow-l" width="70%"><input type="text" name="targetDocName" value="%value targetDocName%" size="20" style="font-size:10pt;width:400"></td>
					</tr>
					%endifvar%
					<!-- 현대제철 냉연공장 전용 -->
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							<select id="requestResponseType" name="requestResponseType" style="width:200" onchange="socketProcessUrlDisplay2(this.value)">
								...
								<option value="onlyRequest">Only Request</option>
								<option value="response">Response</option>
							</select>&nbsp;Response Doc Name&nbsp;<input type="text" name="resDocName" value="%value resDocName%" size="20" style="font-size:10pt;width:90">
							<script language="javascript">
					...
					<!--
					%ifvar docInterfaceType equals('REST API To Socket')%
					<tr style="display:" id="socketProcessServiceUrlTr">
						<td class="evenrow" width="30%">Socket Processing Service URL</td>
						<td class="evenrow-l" width="70%"><input type="text" name="socketProcessServiceUrl" value="%value socketProcessServiceUrl%" size="20" style="font-size:10pt;width:400"></td>
					</tr>
					%else%
					<tr style="display:none" id="socketProcessServiceUrlTr">
						<td class="evenrow" width="30%">Socket Processing Service URL</td>
					...
					%endifvar%
					-->
					%ifvar socketProcessServiceUrl -notempty%
					<tr style="display:" id="socketProcessServiceUrlTr">
						<td class="evenrow" width="30%">Socket Processing Service URL</td>
						<td class="evenrow-l" width="70%"><input type="text" name="socketProcessServiceUrl" value="%value socketProcessServiceUrl%" size="20" style="font-size:10pt;width:400"></td>
					</tr>
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							%ifvar customerCode equals('HSC')%
							    ...
							    Answerback Doc ID&nbsp;<input type="text" name="ackDocID" value="%value ackDocID%" size="20" style="font-size:10pt;width:100">&nbsp;&nbsp;&nbsp;&nbsp;
								Answerback Socket Name&nbsp;
								<select name="ackSocketName" style="width:400">
									<option value="">선택</option>
									%loop SocketNameDescList%
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							<script language="javascript">
								document.pform.targetService.value = "%value targetService%";
							</script>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							Answerback Doc Data&nbsp;<input type="text" name="ackDocData" value="%value ackDocData%" size="20" style="font-size:10pt;width:100">&nbsp;&nbsp;&nbsp;&nbsp;
							Answerback Receive Retry Count&nbsp;<input type="text" name="ackRetryCount" value="%value ackRetryCount%" size="20" style="font-size:10pt;width:100">						
						</td>
					</tr>					
					<tr>
						<td class="evenrow" width="30%">Messaging Use YN</td>
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							<script language="javascript">
								var sourceSystemNames = document.pform.tempSourceSystemName;
								var sLength = sourceSystemNames.options.length;
								var ssNames = "%value sourceSystemName%";
								
								if (ssNames != "") {
									var ssNameList = ssNames.split("/");
									
									for (var i = 0; i < ssNameList.length; i++) {
										for (var j = 0; j < sLength; j++) {
											if (sourceSystemNames.options[j].value == ssNameList[i]) {
												sourceSystemNames.options[j].selected = true;
												break;
											}
										}
									}
					...
					<tr>
						<td class="evenrow" width="30%">External API URL</td>
						<td class="evenrow-l" width="70%"><input type="text" name="externalApiUrl" value="%value externalApiUrl%" size="20" style="font-size:10pt;width:400"></td>
					</tr>
					<tr>
						<td class="evenrow" width="30%">Convert JSON YN</td>
						<td class="evenrow-l" width="70%">
					...
					<tr>
						<td class="evenrow" width="30%">Doc Length</td>
						<td class="evenrow-l" width="70%"><input type="text" id="docLength" name="docLength" value="%value docLength%" size="20" style="font-size:10pt;width:400">&nbsp;<input type="button" name="SUBMIT" value="Calculate Doc Length"  onclick="calDocLength()"></input></td>
					</tr>
					<tr>
						<td class="evenrow" width="30%">Change Sending Doc Length</td>
						<td class="evenrow-l" width="70%">
				...
				%else%
					...
					<table class="tableForm" width="100%">
						...
						<tr>
							...
							<td class="evenrow-l" width="70%">
								<select name="sPlantCode" style="width:300">
								...
								<script language="javascript">
									document.pform.sPlantCode.value = "%value sPlantCode%";
								</script>
							</td>
						</tr>
						%endifvar%
					...
					<input type="hidden" id="filePath" name="filePath" value="%value filePath%">
					<input type="hidden" name="fileName" value="">
					<input type="hidden" id="moveAction" name="moveAction" value="/JSocketAdapter/basic-docinterfaceid.dsp">
						
					<table class="tableForm" width="100%">					
					<tr>
						<td class="heading" colspan="12">Interface Doc Information</td>
					</tr>
					...
					%ifvar -notempty DocInterfaceIDConfig/Items%
						%loop DocInterfaceIDConfig/Items%
							<tr>
								...
								%ifvar ../schemaDefDownload equals('true')%
									%ifvar schemaDefineFileExists equals('true')%
										<td class="evenrowdata">
											<a href="javascript:doAction('download', '%value interfaceID%_%value docName%.%value ../docFileExtension%')"><img src="%value ../../designPath%icons/movedown_enabled.png" alt="Download" border="0"></a>
										</td>
									%else%
										<td class="evenrowdata">
											<a href="#" title="%value interfaceID%_%value docName%.%value ../docFileExtension% 파일이 존재하지 않습니다."><img src="%value ../../designPath%icons/movedown_enabled.png" alt="Download" border="0"></a>
										</td>
									%endifvar%
								<!--
								<td class="evenrowdata">
									<a href="javascript:downloadSchemaDefine('%value interfaceID%_%value docName%.%value ../docFileExtension%')"><img src="%value ../../designPath%icons/movedown_enabled.png" alt="Download" border="0"></a>
								</td>
								-->
								%endifvar%
								<td class="evenrowdata">
						...
						%endloop%
					%else%
						<tr><td colspan=12>No interface docs are currently registered.</td></tr>
					%endifvar%
				</table>
				%endifvar%
			%endifvar%
		...
		<!-- 요청전문, 응답전문 파싱결과 팝업창 띄우기 샘플 -->
		<!-- javascript 이용 샘플
		<form name="requestDocForm" method="post">
			<input type="hidden" name="todo" value="docDataView">
			<input type="hidden" name="docName" value="0700100">
			<input type="hidden" name="parsingCharSet" value="KSC5601">
			<input type="hidden" name="docData" value="APITEST00490700100NBLEKLIA20220916132526a111111111b222222222">
			<input type="button" name="DOCDATAVIEW" value="Doc Data View"  onclick="return docDataView('request')"></input>
		</form>
		
		<form name="responseDocForm" method="post">
			<input type="hidden" name="todo" value="docDataView">
			<input type="hidden" name="docName" value="0710100">
			<input type="hidden" name="parsingCharSet" value="KSC5601">
			<input type="hidden" name="docData" value="APITEST00690710100KLIANBLE20220919062742a111111111b222222222021aa1bb1cc2aa2bb2cc">
			<input type="button" name="DOCDATAVIEW" value="Doc Data View"  onclick="return docDataView('response')"></input>
		</form>
		-->
		
		<!-- jquery 이용 샘플
		<form id="requestDocForm" name="requestDocForm" method="post">
			<input type="hidden" name="todo" value="docDataView">
			<input type="hidden" name="docName" value="0700100">
			<input type="hidden" name="parsingCharSet" value="KSC5601">
			<input type="hidden" name="docData" value="APITEST00490700100NBLEKLIA20220916132526a111111111b222222222">
			<input type="button" name="DOCDATAVIEW" value="Doc Data View"  onclick="return docDataView2('request')"></input>
		</form>
		
		<form id="responseDocForm" name="responseDocForm" method="post">
			<input type="hidden" name="todo" value="docDataView">
			<input type="hidden" name="docName" value="0710100">
			<input type="hidden" name="parsingCharSet" value="KSC5601">
			<input type="hidden" name="docData" value="APITEST00690710100KLIANBLE20220919062742a111111111b222222222021aa1bb1cc2aa2bb2cc">
			<input type="button" name="DOCDATAVIEW" value="Doc Data View"  onclick="return docDataView2('response')"></input>
		</form>
		-->
