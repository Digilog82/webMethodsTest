%invoke JSocketAdapter.ADMIN:adminDQLDeploymentSchedule%
...
<html>
	<head>		
		...
		<script language="javascript">
			function doAction(todo, pname){
				...
				if(todo == 'delete'){
					if(confirm("Do you really want to delete schedule of  " + pname)){
						frm.submit();
					}
				} else {
					frm.submit();
	...
	<body onload="javascript:alertMsg()">		
		...
		%ifvar todo -notempty%
			<ul class="listitems">
				<li class="listitem"><a href="as400-reservationschedule.dsp">Return to Deployment Reservation Schedule Information</a></li>
			</ul>
		%else%
		...
		<table class="tableForm" width="100%">	
			%ifvar todo equals('read')%
				<tr>
					<td class="heading" colspan="2">Deployment Reservation Schedule Properties</td>
				</tr>
				<tr>
					<tr>
			...
			%else%
				<tr>
					<td class="heading" colspan=7>Deployment Reservation Schedule Information</td>
				</tr>
				...
				%ifvar -notempty onceTasks%
					...
					%endloop%
				%else%
					<tr><td colspan="7">No reservation schedules are currently registered.</td></tr>
				%endifvar%
			%endifvar%
		</table>
		</form>
