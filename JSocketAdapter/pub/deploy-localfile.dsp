...
<title>Deploy Business Name</title>
		...
		<script language="javascript">
			function doAction(todo) {				
				...
				if (todo == 'search') {
					...
					if (document.pform.selectedList.value == "") {
						alert("Deploy할 Local File Path를 선택하십시요!!!");
						return;
					} else {
						document.pform.submit();
					}
...
<body onload="javascript:alertMsg()">
		...
		%ifvar deploy equals('remote')%
			...
			<ul class="listitems">
				<li class="listitem"><a href="deploy-localfile.dsp?deployClass=%value deployClass%&searchType=noDeploy&dupProcessType=notAdd">Return to Delete Local File Deployment Information</a></li>
			</ul>
			
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Delete Local File Deployment Result</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
        	...
        	<tr>
						<td class="heading" colspan=5>Delete Local File Information</td>
					</tr>
       ...
       %endifvar%
		...
		%else%
			...
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Delete Local File Deployment</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="action" colspan="2">
						<input type="button" name="SUBMIT" value="Search Delete Local File"  onclick="return doAction('search')"></input>&nbsp;&nbsp;
						<input type="button" name="SUBMIT" value="Deploy Delete Local File"  onclick="return doAction('remote')"></input>
					</td>
				</tr>
			</table>

      <table class="tableView" width=100%>
        <tr><td class="heading" colspan=6>Local File Path Infomation For Deletion</td></tr>
        <tr class="subheading2">
          ...
          <td>Local File Path</td>
        	...
        	%loop RegInfoList -$index%
						<tr>
							<td class="evenrowdata"><input type="checkbox" name="selectedList%value $index%" value="%value filePath%">
					...
					%endloop%
				%else%
					<tr><td colspan=6>No local file paths to deploying are currently.</td></tr>
      	%endif%
    	</table>
		%endifvar%
	</form>
