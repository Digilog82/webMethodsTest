		...
		<script language="javascript">
			function doAction(todo) {				
				...
				if (todo == 'search') {
					...
					var asisintDomain = "";
					var tobeintDomain = "";
					var asisintDomainVal = "";
					var tobeintDomainVal = "";
					
					for (var i=0; i < document.pform.length; i++) {
						if (document.pform.elements[i].checked) {
							cDocName = document.pform.elements[i].value;
							asisDomain = "asis" + cDocName;
							tobeDomain = "tobe" + cDocName;
							asisDomainVal = document.getElementById(asisDomain).value;
							tobeDomainVal = document.getElementById(tobeDomain).value;
							
							asisintDomain = "asisint" + cDocName;
							tobeintDomain = "tobeint" + cDocName;
							asisintDomainVal = document.getElementById(asisintDomain).value;
							tobeintDomainVal = document.getElementById(tobeintDomain).value;
							
							if (asisDomainVal == "") {
								document.pform.selectedList.value = "";
								alert("Doc Name " + cDocName + "의 This Server IP 또는 Domain을 입력하십시요!!!");
								return;
							}
							...
							if (document.pform.selectedList.value == "") {
								document.pform.selectedList.value = cDocName + "|" + asisDomainVal + "|" + tobeDomainVal + "|" + asisintDomainVal + "|" + tobeintDomainVal;
							} else {
								document.pform.selectedList.value = document.pform.selectedList.value + "$" + cDocName + "|" + asisDomainVal + "|" + tobeDomainVal + "|" + asisintDomainVal + "|" + tobeintDomainVal;
							}
						}
					}					
					
					if (document.pform.selectedList.value == "") {
						alert("Deploy할 Doc Name을 선택하십시요!!!");
						return;
					} else {
						document.pform.submit();
					}
...
<body onload="javascript:alertMsg()">
		...
		%else%
		<div class="message">Target Server에서 URL에 대한 정보(IP/Domain, Port)가 다를 경우 "Change IP or Domain & Port" 항목의 Target Server IP/Domain, Port를 수정하십시요.</div>
		%endifvar%
		...
		%ifvar deploy equals('remote')%
			...
			<ul class="listitems">
				<li class="listitem"><a href="deploy-httpdocinterfaceid.dsp?deployClass=%value deployClass%&searchType=noDeploy&dupProcessType=notAdd">Return to HTTP/S Target Doc Deployment Information</a></li>
			</ul>
			
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">HTTP/S Target Doc Deployment Result</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
        	...
        	<tr>
						<td class="heading" colspan=5>HTTP/S Target Doc Information</td>
					</tr>
       ...
       %endifvar%
		...
		%else%
			...
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">HTTP/S Target Doc Deployment</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="action" colspan="2">
						<input type="button" name="SUBMIT" value="Search HTTP/S Target Doc"  onclick="return doAction('search')"></input>&nbsp;&nbsp;
						<input type="button" name="SUBMIT" value="Deploy HTTP/S Target Doc"  onclick="return doAction('remote')"></input>
					</td>
				</tr>
			</table>
      ...
      <table class="tableView" width=100%>
        <tr>
					<td class="heading" colspan=6>HTTP/S Target Doc Information</td>
				</tr>
        <tr class="subheading2">
					...
					<td>Interface ID</td>
					<td>Description</td>
					<td>Internal API URL<br>Target URL</td>
					<td>Change IP or Domain & Port</td>
        </tr>
				%ifvar -notempty RegInfoList%
					%loop RegInfoList -$index%
						<tr>
							<td class="evenrowdata"><input type="checkbox" name="checkedList%value $index%" value="%value docName%">
							<td>%value docName%</td>
							<td>%value interfaceID%</td>
							<td>%value description%</td>
							<td>Internal API URL : %value internalApiUrl%<br><br>
							    Target URL : %value targetUrl%
						    </td>
							<td>
								This Server <input type="text" disabled id="tmpasisint%value docName%" name="tmpasisint%value docName%nm" value="%value intIpDomain%" size="5" style="font-size:10pt;width:200"> -->
								            <input type="hidden" id="asisint%value docName%" name="asisint%value docName%nm" value="%value intIpDomain%">
								Target Server <input type="text" id="tobeint%value docName%" name="tobeint%value docName%nm" value="%value intIpDomain%" size="5" style="font-size:10pt;width:200"><br><br>
								This Server <input type="text" disabled id="tmpasis%value docName%" name="tmpasis%value docName%nm" value="%value ipDomain%" size="5" style="font-size:10pt;width:200"> -->
								            <input type="hidden" id="asis%value docName%" name="asis%value docName%nm" value="%value ipDomain%">
								Target Server <input type="text" id="tobe%value docName%" name="tobe%value docName%nm" value="%value ipDomain%" size="5" style="font-size:10pt;width:200">
							</td>
						</tr>
					%endloop%
				%else%
					<tr><td colspan=6>No http/s target docs to deploying are currently.</td></tr>
				%endif%
    	</table>
		%endifvar%
	</form>
