...
<html>
	...
	<body>
		...
		%ifvar todo equals('resubmit')%
			%ifvar fullFileName -notempty%
				<form name="dform" method="post">
					<input type="hidden" name="todo" value="read">
					<input type="hidden" name="fullFileName" value="%value fullFileName%">
				</form>
				<script langauge='javascript'>
					alert("Resubmit 처리 성공 !!!");
					document.dform.submit();
				</script>
			%else%
				<script langauge='javascript'>
					alert("Resubmitted Audit Log File을 찾지 못했습니다.\nSocket Log &gt; Local Server Resubmit Log 메뉴에서 처리결과를 조회하시기 바랍니다.");
					window.close();
				</script>
			%endifvar%
		%endifvar%
		...
		<table class="tableForm" width="100%">
			...
			<tr>
				...
				<td class="evenrow-l" width="70%">
					%ifvar SocketAuditInfo/resultCode equals('F')%<font color="red">Fail</font>%else%Success%endifvar%<font color="red">(Resubmitted)</font>
				</td>
			</tr>
			<tr>
