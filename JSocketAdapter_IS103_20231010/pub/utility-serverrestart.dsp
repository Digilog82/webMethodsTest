...
<html>
	...
	<body onload="javascript:alertMsg()">
		...
		<div class="message">현재 Server에서 운영되고 있는 기능들을 다른 Server에서 실행되도록 Switching 한 후 현재 Server를 Shutdown or Restart 합니다.</div>
		...
		<table class="tableForm" width="100%">
			...
			%else%
              ...
              %loop listeners%
                ...
                %endif%
              %endloop%
            </select> (인터페이스용으로 사용하지 않는 Port를 선택)
            <input type="hidden" name="operation" value="setPrimary">
            <input type="hidden" name="asisPrimaryPort" value="%value primary/key encode(htmlattr)%">
            <input type="hidden" name="asisPrimaryPkg" value="%value primary/pkg encode(htmlattr)%">
				  </td>
				</tr>
				<tr>
				  <!--
		  ...
		  <table class="tableForm" width="100%">
            ...
            5. Enabled 상태의 Regular Port Disable<br>
            ...
            6. Enabled 상태의 Registration Internal Port Disable<br>
               &nbsp;&nbsp;&nbsp;&nbsp;Disable 처리 이전에 수신되고 아직 완료가 안된 서비스는 정상적으로 처리되어 Client에게 정상적으로 응답을 보낸다.<br>
               &nbsp;&nbsp;&nbsp;&nbsp;Client 쪽에서도 Port Disable 하더라도 Connection이 끊기지 않고 정상적으로 응답을 수신한다.<br>
            ...
            13. Disable 처리했던 Registration Internal Port Enable<br>
            14. Disable 처리했던 Regular Port Enable<br>
