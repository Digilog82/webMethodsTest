		...
		<script language="javascript">
			function doAction(todo) {				
				...
				if (todo == 'search') {
					...
					if (document.pform.selectedList.value == "") {
						alert("Deploy할 Trigger Name을 선택하십시요!!!");
						return;
					} else {
						document.pform.submit();
					}
...
<body onload="javascript:alertMsg()">
		...
		%ifvar deploy equals('remote')%
			...
			<ul class="listitems">
				<li class="listitem"><a href="deploy-trigger.dsp?deployClass=%value deployClass%&searchType=noDeploy&dupProcessType=notAdd">Return to Online Trigger Deployment Information</a></li>
			</ul>
			
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Online Trigger Deployment Result</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
        	...
        	<tr>
						<td class="heading" colspan=5>Online Trigger Information</td>
					</tr>
       ...
       %endifvar%
		...
		%else%
			...
			<table class="tableForm" width="100%">
				<tr>
					<td class="heading" colspan="2">Online Trigger Deployment</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="action" colspan="2">
						<input type="button" name="SUBMIT" value="Search Online Trigger"  onclick="return doAction('search')"></input>&nbsp;&nbsp;
						<input type="button" name="SUBMIT" value="Deploy Online Trigger"  onclick="return doAction('remote')"></input>
					</td>
				</tr>
			</table>
        ...
        <tr class="subheading2">
				...
				%ifvar -notempty RegInfoList%
					%loop RegInfoList -$index%
						<tr>
							<td class="evenrowdata"><input type="checkbox" name="selectedList%value $index%" value="%value trgName%">
							<td>%value trgName%</td>
					...
					%endloop%
				%else%
					<tr><td colspan=4>No online triggers to deploying are currently.</td></tr>
				%endif%
    	</table>
		%endifvar%
	</form>
