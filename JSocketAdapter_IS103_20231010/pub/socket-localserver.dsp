	...
	<head>		
		...
		<script language="javascript">
			...
			function checkServiceNode(aTodo) {
				var jmsCreationInfo = "";
				var tempJmsCreationInfos = document.pform.tempJmsCreationInfos;
				var hNLength = tempJmsCreationInfos.options.length;
				
				if (hNLength > 0) {
					for (var i=0; i < hNLength; i++) {
						if (jmsCreationInfo == "") {
							jmsCreationInfo = tempJmsCreationInfos.options[i].value;
						} else {
							jmsCreationInfo = jmsCreationInfo + "§" + tempJmsCreationInfos.options[i].value;
						}
					}
					
					document.pform.jmsCreationInfo.value = jmsCreationInfo;
				}
			...
			function doAction(todo, pnum){
				...
				if (todo == 'delete') {
					...
					*/
					...
					var tempJmsCreationInfos = document.pform.tempJmsCreationInfos;
					var hLength2 = tempJmsCreationInfos.options.length;
					...
					document.pform.jmsCreationInfo.value = "";
					
					if (hLength2 == 0) {
					} else if (hLength2 == 1) {
						frm.jmsCreationInfo.value = tempJmsCreationInfos.options[0].value;
					} else {					
						// Topic, JMS Trigger Creation List 전체를 선택한 것으로 처리
						for (var i = 0; i < hLength2; i++) {
							tempJmsCreationInfos.options[i].selected = true;
						}
					}
			...
			/* 2022.08.15 수정. iframe을 사용하지 않고 JQuery를 사용하는 방식으로 변경
			...
			*/
			...
			function changeKeyIv() {
				...
				if (alg == "") {
					alert("Encrypt Algorithm을 먼저 선택하십시요.");
					return;
				}
				
				$.ajax({
					type : "POST",
					url : "/invoke/JSocketAdapter.COMMON.UTIL.FLOW/createStringKeyIv",
					cache : false,
					data : "alg=" + alg,
					dataType : "json",
					async : true,
					success : function(result) {
								setChangedKeyIv(result.sk, result.sk24, result.sk32, result.iv);
							  },
					error : function() {
								alert("Encrypt Key, Encrypt Initial Vector를 생성하는 중에 에러가 발생하였습니다 !!!");
					        }
				});
			};
			...
			function enableCharList(flag) {
				if (flag == "Yes") {
					document.pform.charListToZero.disabled = false;
				} else {
					document.pform.charListToZero.disabled = true;
				}
			}
			...
			function applyDefaultName() {
				var online = document.pform.online.value;
				var portNumber = document.pform.portNumber.value;
				var customerCode = "%value customerCode%";
				var topicName = "";
				var jmsTrgName = "";
				
				if (portNumber == "") {
					alert("Port Number를 먼저 입력하십시요.");
					return;
				}
				...
				if (customerCode == "HCC") { // 현대제철 냉연공장 전용 ==> 공장코드를 삽입한다.
					var plantCode = document.pform.plantCode.value;
					
					if (plantCode == "") {
						alert("Plant를 먼저 입력하십시요.");
						return;
					}
					
					topicName = "/topic/tcp/server/" + online + "/" + plantCode + "/" + portNumber;
					jmsTrgName = "trgJMS_SERVER_" + plantCode + "_" + portNumber;
				} else {
					topicName = "/topic/tcp/server/" + online + "/" + portNumber;
					jmsTrgName = "trgJMS_SERVER_" + portNumber;
				}
				
				document.pform.tempTopicName.value = topicName;
				document.pform.tempJmsTrgName.value = jmsTrgName;
			}
			
			function addJmsCreation() {
				var tempJmsCreationInfos = document.pform.tempJmsCreationInfos;
				var tempJmsConnectionAlias = document.pform.tempJmsConnectionAlias.value;
				var tempSrcToTgt = document.pform.tempSrcToTgt.value; // 필수 입력 항목 아님
				var tempTopicName = document.pform.tempTopicName.value;
				var tempJmsTrgFolder = document.pform.tempJmsTrgFolder.value;
				var tempJmsTrgName = document.pform.tempJmsTrgName.value;
				var tempJmsTrgService = document.pform.tempJmsTrgService.value;
				var tempJmsTrgProcessingMode = document.pform.tempJmsTrgProcessingMode.value;
				
				if (tempJmsConnectionAlias == "") {
					alert("JMS Connection Alias를 선택하십시요.");
					return;
				}
				...
				if (tempTopicName == "") {
					alert("Topic Name을 입력하십시요.");
					return;
				}
				
				if (tempJmsTrgFolder == "") {
					alert("JMS Trigger Folder를 입력하십시요.");
					return;
				}
				
				if (tempJmsTrgName == "") {
					alert("JMS Trigger Name을 입력하십시요.");
					return;
				}
				
				if (tempJmsTrgService == "") {
					alert("JMS Trigger Service를 입력하십시요.");
					return;
				}
								
				var jmsInfo = tempJmsConnectionAlias + "|" + tempSrcToTgt + "|" + tempTopicName + "|" + tempJmsTrgFolder + "|" + tempJmsTrgName + "|" + tempJmsTrgService + "|" + tempJmsTrgProcessingMode;
				
				// Topic, JMS Trigger Creation 중복 체크
				var hNLength = tempJmsCreationInfos.options.length;
				for (var i=0; i < hNLength; i++) {
					var hNV = tempJmsCreationInfos.options[i].value;
					var hNVs = hNV.split("|");
					var hN0 = hNVs[0];
					var hN1 = hNVs[1];
					var hN2 = hNVs[2];
					
					if (tempJmsConnectionAlias == hN0 && tempSrcToTgt == hN1 && tempTopicName == hN2) {
						alert("Topic Name " + tempTopicName + "는(은) 이미 추가되어 있습니다.");
						return;
					}
				}
				
				tempJmsCreationInfos.options[hNLength] = new Option(jmsInfo, jmsInfo);
			}
			
			function deleteJmsCreation() {
				var tempJmsCreationInfos = document.pform.tempJmsCreationInfos;
				var hNLength = tempJmsCreationInfos.options.length;
				...
				for (var i = 0; i < hNLength; i++) {
					if (tempJmsCreationInfos.options[optionIndex].selected) {
						tempJmsCreationInfos.options.remove(optionIndex);
						selectYN = "true";
					} else {
						optionIndex++;
					}
					
					newValueLength = tempJmsCreationInfos.options.length;
				...
				if (selectYN == "false") {
					alert("삭제할 Topic, JMS Trigger Creation 정보를 선택하십시요.");
					return;
				}
			}
			
			function updateJmsCreation() {
				var tempJmsCreationInfos = document.pform.tempJmsCreationInfos;
				var hNLength = tempJmsCreationInfos.options.length;
				var tempJmsConnectionAlias = document.pform.tempJmsConnectionAlias.value;
				var tempSrcToTgt = document.pform.tempSrcToTgt.value; // 필수 입력 항목 아님
				...
				for (var i=0; i < hNLength; i++) {
					...
					if (tempJmsConnectionAlias == hN0 && tempSrcToTgt == hN1 && tempTopicName == hN2) {
						if (confirm("Topic Name " + tempTopicName + "는(은) 이미 추가되어 있습니다.\nJMS Trigger 정보만 수정하시겠습니까?")) {
							break;
						} else {
							return;
						}
				...
				for (var i = 0; i < hNLength; i++) {
					if (tempJmsCreationInfos.options[optionIndex].selected) {
						tempJmsCreationInfos.options[optionIndex] = new Option(jmsInfo, jmsInfo);
						selectYN = "true";
						break;
					} else {
						optionIndex++;
				...
				if (selectYN == "false") {
					alert("수정할 Topic, JMS Trigger Creation 정보를 먼저 선택하십시요.");
					return;
				}
			}
			
			function setUpdateJmsCreation(jmsInfo) {
				var hNVs = jmsInfo.split("|");
				var tempJmsConnectionAlias = hNVs[0];
				var tempSrcToTgt = hNVs[1];
				var tempTopicName = hNVs[2];
				var tempJmsTrgFolder = hNVs[3];
				var tempJmsTrgName = hNVs[4];
				var tempJmsTrgService = hNVs[5];
				var tempJmsTrgProcessingMode = hNVs[6];
				
				if (tempSrcToTgt == "NA") {
					tempSrcToTgt = "";
				}
								
				document.pform.tempJmsConnectionAlias.value = tempJmsConnectionAlias;
				document.pform.tempSrcToTgt.value = tempSrcToTgt;
				document.pform.tempTopicName.value = tempTopicName;
				document.pform.tempJmsTrgFolder.value = tempJmsTrgFolder;
				document.pform.tempJmsTrgName.value = tempJmsTrgName;
				document.pform.tempJmsTrgService.value = tempJmsTrgService;
				vdocument.pform.tempJmsTrgProcessingMode.value = tempJmsTrgProcessingMode;
			}
	...
	<body onload="javascript:alertMsg()">
		...
		<table class="tableForm" width="100%">	
			%ifvar todo equals('create')%
				...
				<tr>
					<td class="evenrow">Source System Name</td>
					<td class="evenrow-l">							
						<select name="srcSystemName" style="width:400">
							<option value="">선택</option>
				...
				<tr>
					<td class="evenrow">Target System Name</td>
					<td class="evenrow-l">							
						<select name="tgtSystemName" style="width:400">
							<option value="">선택</option>
							%loop SystemNameDescList%
								<option value="%value systemNameValue%">%value systemNameDisplay%</option>
							%endloop%
				...
				<tr>
					<td class="evenrow" width="30%">Null To Space YN</td>
					<td class="evenrow-l" width="70%">
						<select name="nullToSpace" style="width:100">
							%ifvar customerCode equals('HCC')%
								<option value="Yes">Yes</option>
								<option value="No">No</option>
							%else%
				...
				%ifvar customerCode equals('HCC')%
				<tr>
					<td class="evenrow" width="30%">Char To Zero YN</td>
					<td class="evenrow-l" width="70%">
						<select name="charToZero" style="width:100" onchange="javascript:enableCharList(this.value)">
							<option value="No">No</option>
							<option value="Yes">Yes</option>
						</select>&nbsp;&nbsp;&nbsp;&nbsp;
						Char List&nbsp;<input type="text" name="charListToZero" value="*/#" disabled size="6" style="font-size:10pt;width:240">
					</td>
				</tr>
				%endifvar%
				<!-- 현대제철 냉연공장 전용 -->
				...
				<tr>
					<td class="evenrow">Doc Delimiter</td>
					<td class="evenrow-l"><input type="text" name="docDelimiter" size="6" style="font-size:10pt;width:400"></td>
				</tr>
				
				<!-- 현대제철 냉연공장 전용 -->
				%ifvar customerCode equals('HCC')%
				<tr>
					<td class="evenrow">Header Remove Length</td>
					<td class="evenrow-l"><input type="text" name="headerRemoveLength" size="6" style="font-size:10pt;width:400"></td>
				</tr>
				<tr>
					<td class="evenrow">Target Header Creation Service</td>
					<td class="evenrow-l">							
						<select name="tgtHeaderCreationService" style="width:400">
							<!--<option value="">선택</option>-->
							%loop tgtHeaderCreationServiceList%
								<option value="%value tgtHeaderCreationServiceList%">%value tgtHeaderCreationServiceList%</option>
							%endloop%
						</select>
					</td>
				</tr>
				...
				<tr>
					<td class="evenrow">Doc Length Type</td>
					<td class="evenrow-l">
						<select name="docLengthType" style="width:400">
							<option value="afterDocLengthField">After Doc Length Field</option>
							<option value="exceptCommonHeader">Except Common Header</option>
							<option value="docAll">Doc All</option>
				...
				<tr>
					<td class="evenrow" width="30%">Doc Receive Interval</td>
					<td class="evenrow-l"><input type="text" name="docReceiveInterval" size="6" style="font-size:10pt;width:400"> (ms)</td>
				</tr>
				<tr>
					<td class="evenrow" width="30%">Topic, JMS Trigger Creation</td>
					<td class="evenrow-l"><input type="button" name="APPLYNAME" value="Apply Default Name"  onclick="return applyDefaultName()"></input>&nbsp;&nbsp;
					                      JMS Connection Alias&nbsp;<select name="tempJmsConnectionAlias" style="width:300">
							                                        %loop jmsUMList%
																		<option value="%value jmsUMList%">%value jmsUMList%</option>
							                                        %endloop%
						                                            </select>&nbsp;&nbsp;
										  Source System Code + "TO" + Target System Code&nbsp;<input type="text" name="tempSrcToTgt" size="6" style="font-size:10pt;width:150"><br><br>
										  Topic : Name&nbsp;<input type="text" name="tempTopicName" size="6" style="font-size:10pt;width:400"><br><br>
										  JMS Trigger : Folder&nbsp;<input type="text" name="tempJmsTrgFolder" size="6" style="font-size:10pt;width:160">&nbsp;&nbsp;
										                Name&nbsp;<input type="text" name="tempJmsTrgName" size="6" style="font-size:10pt;width:160">&nbsp;&nbsp;
														Service&nbsp;<input type="text" name="tempJmsTrgService" size="6" style="font-size:10pt;width:300">&nbsp;&nbsp;
														Processing Mode&nbsp;<select name="tempJmsTrgProcessingMode" style="width:80">
																				<option value="Serial">Serial</option>
																				<option value="Concurrent">Concurrent</option>
																			 </select><br><br>
										  <input type="button" name="ADDVALUE" value="Add Value"  onclick="return addJmsCreation()"></input>&nbsp;
							              <input type="button" name="UPDATEVALUE" value="Update Value"  onclick="return updateJmsCreation()"></input>
					</td>
				</tr>
				<tr>
					<td class="evenrow" width="30%">Topic, JMS Trigger Creation List</td>
					<td class="evenrow-l" width="70%">
						<select name="tempJmsCreationInfos" style="width:400;overflow:auto" size=5 multiple onclick="setUpdateJmsCreation(this.value)">
						</select>&nbsp;
						<input type="button" name="DELETEVALUE" value="Delete Value"  onclick="return deleteJmsCreation()"></input>
						<input type="hidden" name="jmsCreationInfo" value="">
					</td>
				</tr>
				<tr>
					<td class="evenrow" width="30%">Encrypt Algorithm</td>
					<td class="evenrow-l" width="70%">
						<select id="algorithm" name="algorithm" style="width:100" onchange="javascript:setKeyIv()">
			...
			%else%
				%ifvar todo equals('read')%
					...
					<tr>
						...
						<td class="evenrow-l">							
							...
							<script language="javascript">
								document.pform.srcSystemName.value = "%value srcSystemName%";
							</script>
						</td>
					</tr>
					<tr>
						...
						<td class="evenrow-l">							
							...
							<script language="javascript">
								document.pform.tgtSystemName.value = "%value tgtSystemName%";
							</script>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							<script language="javascript">
								document.pform.nullToSpace.value = "%value nullToSpace%";
							</script>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							...
							<script language="javascript">
								document.pform.charToZero.value = "%value charToZero%";
							</script>
							%ifvar charToZero equals('Yes')%
								Char List&nbsp;<input type="text" name="charListToZero" value="%value charListToZero%" size="6" style="font-size:10pt;width:240">
							%else%
								Char List&nbsp;<input type="text" name="charListToZero" value="*/#" disabled size="6" style="font-size:10pt;width:240">
							%endifvar%
						</td>
					...
					<tr>
						<td class="evenrow">Doc Delimiter</td>
						<td class="evenrow-l"><input type="text" name="docDelimiter" value="%value docDelimiter%" size="6" style="font-size:10pt;width:400"></td>
					</tr>
					...
					<tr>
						<td class="evenrow">Header Remove Length</td>
						<td class="evenrow-l"><input type="text" name="headerRemoveLength" value="%value headerRemoveLength%" size="6" style="font-size:10pt;width:400"></td>
					</tr>
					<tr>
						<td class="evenrow">Target Header Creation Service</td>
						<td class="evenrow-l">							
							...
							<script language="javascript">
								document.pform.tgtHeaderCreationService.value = "%value tgtHeaderCreationService%";
							</script>
						</td>
					</tr>
					%endifvar%
					...
					<tr>
						...
						<td class="evenrow-l">
							...
							<script language="javascript">
								document.pform.docLengthType.value = "%value docLengthType%";
							</script>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td class="evenrow" width="30%">Doc Receive Interval</td>
						<td class="evenrow-l"><input type="text" name="docReceiveInterval" value="%value docReceiveInterval%" size="6" style="font-size:10pt;width:400"> (ms)</td>
					</tr>
					<tr>
						<td class="evenrow" width="30%">Topic, JMS Trigger Creation</td>
						<td class="evenrow-l"><input type="button" name="APPLYNAME" value="Apply Default Name"  onclick="return applyDefaultName()"></input>&nbsp;&nbsp;
					...
					<tr>
						...
						<td class="evenrow-l" width="70%">
							<select name="tempJmsCreationInfos" style="width:400;overflow:auto" size=5 multiple onclick="setUpdateJmsCreation(this.value)">
								%loop jmsCreationInfos%
									<option value="%value jmsCreationInfos%">%value jmsCreationInfos%</option>
								%endloop%
							</select>&nbsp;
							<input type="button" name="DELETEVALUE" value="Delete Value"  onclick="return deleteJmsCreation()"></input>
							<input type="hidden" name="jmsCreationInfo" value="">
				...
				%else%
					...
					%endifvar%
					<tr>
						<td class="heading" colspan=13>Local Server Socket Information</td>
					</tr>
					...
					%ifvar -notempty ServerPortConfig/Ports%
						%loop ServerPortConfig/Ports -$index%
							<tr>
								<td class="evenrowdata"><input type="checkbox" name="selectedList%value $index%" value="%value portNumber%"></td>
						...
						%endloop%
					%else%
						<tr><td colspan=13>No local server sockets are currently registered.</td></tr>
					%endifvar%
					</table>
				%endifvar%
			%endifvar%
