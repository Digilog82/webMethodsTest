...
<SCRIPT>
    String.prototype.trim = function () {
     ...
     function showClientSecret(secret,message) {
        var msg = message+"\n"+secret;
        ...
        if ( confirm(msg)) {
            if(window.location.protocol == 'http:') {
                ...
                hiddenField.value=secret;
                hiddenField.style.top = "0";
                hiddenField.style.left = "0";
                ...
                try {
                    ...
                    alert('Error copying client secret. Please try this over secure protocol access');
                 } finally {
  ...
  <TABLE width="100%">
   <TR>
    ...
    %ifvar operation equals('update-status')%
            ...
            %endinvoke%
    %endif%     
    %ifvar operation equals('update-secret')%
        %invoke wm.server.oauth:updateClientSecret%
        %endinvoke%
    %endif%
   <TR>
      <TD colspan="2">
...
%ifvar client_secret -notempty%
	<script>
		showClientSecret('%value client_secret%','%value copymessage%');
	</script>
%endif%
</HTML>
